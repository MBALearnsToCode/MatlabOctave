...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % This is the only function that you're expected to change. Right now, it just returns a lot of zeros, which is obviously not the correct output. Your job is to change that.
   ...
   numHid = size(model.input_to_hid, 1);
   ffNN = class_ffNN(256, [numHid 10], ...
      {'logisticNoBias' 'softmaxNoBias'}, false, false);  
   ffNN.weights{1} = model.input_to_hid';
   ffNN.weights{2} = model.hid_to_class';
   weightGrads = fProp_bProp(ffNN, data.inputs', ...
      data.targets', false, {{'L2'} wd_coefficient});
   
   ret.input_to_hid = weightGrads{1}';
   ret.hid_to_class = weightGrads{2}';
end
