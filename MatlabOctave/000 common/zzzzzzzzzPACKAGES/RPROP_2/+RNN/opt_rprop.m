...
D = RNN.class2matrix(train_out,nn.nlabels);
...
if testmode
    ...
    D_test = RNN.class2matrix(test_out,nn.nlabels);
end
...
for Iter = 1:p.MaxIter
    ...
    a = RNN.computenetwork(nn,train_in,1);
    
    err = (D-a{nn.nl_regr});
    E   = RNN.MSE(D,a{nn.nl_regr})/(n_sample*n_output);
    
    if testmode
        a_test = RNN.computenetwork(nn,test_in,1);
        E_test = RNN.MSE(D_test,a_test{nn.nlayers-1})/(n_sample_test*n_output);
    end
    ...
    % Compute statistics
    train = RNN.class_stat(a{nn.nlayers},train_out,nn.nlabels);
    stats.mse(Iter)                 = E;
    ...
    if testmode
        test = RNN.class_stat(a_test{nn.nlayers},test_out,nn.nlabels);
