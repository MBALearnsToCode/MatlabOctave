...
1. The #1 criterion for judging a supervised ML model is whether it explains/interprets what's going on in the Training data set
...
2. LOGISTIC REGRESSION: "signal" $z_i = w_0 \cdot 1 + w_1 x_{1i} + w_2 x_{2i} + \dots + w_k x_{ki}$, hypothetical prediction $h_i = f(z_i)$. Why is $h_i = \frac {1} {1 + \text{exp}(-z_i)}$ a good function for Logistic Regression? Why is $c_i = - y_i \text{ln} h_i - (1 - y_i) \text{ln} (1 - h_i)$ a good cost function?
