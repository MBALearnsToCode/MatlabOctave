function [ffNN_updated ...
   ...
   stepRate_init = 1, stepRate_adaptDownUp = [0.5 1.2], ...
   stepRate_range = [1e-6 50.0], ...
   decayRate_init = 9e-1, momentumRate_init = 9e-1, ...    
   nesterov = true, weightRegulArgs_list = {{'L2'} [0]}, ...
   bestStop = true, ffNN_weightGradRMS_init = {}, ...
   ...
   if isempty(ffNN_weightGradRMS_init)
      ffNN_weightGradRMS_updated = ...
         zeros_weightDimSizes(ffNN_updated);
   else
      ffNN_weightGradRMS_updated = ...
         ffNN_weightGradRMS_init;
   endif
   trainCostAvg_exclWeightPenalty_approx = ...
      validCostAvg_exclWeightPenalty = ...
...
fprintf('\n\nTRAIN FORWARD-FEEDING NEURAL NETWORK BY "RMSPROP":\n\n');
   fprintf('   DATA SETS:\n');
...
fprintf('      Training Batches per Epoch: %i batches of %i', ...
   ...
   fprintf('      Step Rate: %g, adapt x%s, range %s\n', ...
      stepRate, mat2str(stepRate_adaptDownUp), ...
      mat2str(stepRate_range));
   fprintf('      RMS Decay Rate %g\n', decayRate);   
   if (momentumRate)
      fprintf('      Momentum: %g', momentumRate);
      if (nesterov)
         fprintf(',   applying Nesterov method\n');
      else
         fprintf('\n');
      endif
   endif
