...
for i=2:nn.nlayers
    ...
    if ~strcmp(nn.typelayer{i-1},'class')
        ...
        if ~strcmp(nn.typelayer{i},'class')
            ...
            out{i}=[RNN.computelayer(out{i-1},[nn.W{i-1};nn.B{i-1}],nn.typelayer{i-1}) ones(ninputs,1)];
        else
            out{i}=RNN.computelayer(out{i-1},[nn.W{i-1};nn.B{i-1}],nn.typelayer{i-1});
        end
    else
        out{i}=RNN.computelayer(out{i-1},nn.W{i-1},nn.typelayer{i-1});
    end
