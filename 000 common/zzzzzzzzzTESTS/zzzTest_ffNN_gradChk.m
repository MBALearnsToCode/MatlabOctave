function results = zzzTest_ffNN_gradChk...
   ...
   for (r = 1 : numRuns)
      ...
      if isempty(ffNN_given)
         ...
         if (featEmbed)         
            ...
            transformFuncs{1} = randElem...
               ...
               tanh_transformFuncHandles(addBias) ...
         ...
         numTransforms = ...
            1 + unidrnd(maxNumLayersExclInputOrEmbed);
         
         for (k = 2 : numTransforms)
      ...
      for (l = 1 : ffNN.numTransforms)
         w = ffNN.weights;
         connectivitiesOnOff{l} = rand(size(w{l})) > 0.5;
      endfor
      ...
      % RANDOMIZE CLASS SKEWNESSES
      % --------------------------
      classSkewnesses = 2 * rand(1, nO);
      
      % COMPUTE ANALYTIC GRADIENTS
      % --------------------------
      [weightGrads, ~, ~, ~, ~, ~, weightRegulParams] = ...
         fProp_bProp(ffNN, input_Arr, ...
         targetOutputs_rowMats_args, false, classSkewnesses, ...
         {regulFuncs regulParams}, true, connectivitiesOnOff);
      aGrad = tests{r}.aGrad = ...
