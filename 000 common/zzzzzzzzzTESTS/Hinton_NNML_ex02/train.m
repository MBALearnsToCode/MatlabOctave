...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    takeTime_stop = time;
    takeTime.fProp_bProp += takeTime.fProp = ...
  ...
  %  CE_chk = costFuncAvg_CrossEntropySoftmax_Mat_RowWise(...
   %    output_layer_state' + tiny, expanded_target_batch').value;
   % fprintf('\ndiff = ');
   % CE_chk - CE
   % pausePressKey;
...
word_embedding_weights_gradient = word_embedding_weights_gradient + ...
  ...
  (back_propagated_deriv_2(1 + (w - 1) * numhid1 : w * numhid1, :)');
    ...
    takeTime_stop = time;
    takeTime.fProp_bProp += takeTime.bProp = ...
       (takeTime_stop - takeTime_start) / timeScale;
