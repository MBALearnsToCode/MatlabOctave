...
fprintf('\n\nTRAIN RESTRICTED BOLTZMANN MACHINE (RBM) (METHOD: GRADIENT DESCENT):\n\n'); 
   fprintf('   DATA SETS:\n');
   ...
   fprintf('\n   TRAINING SETTINGS:\n');   
   fprintf('      Training Epochs: %s, with Constrastive Divergence Markov Chain Lengths %s\n', ...
      mat2str(trainNumsEpochs), mat2str(cd_chainLengths));
   ...
   for (epoch = 1 : trainNumEpochs)
      ...
      for (batch = 1 : trainNumBatches)
         ...
         weightGrads = cd(rbm_temp, trainData_batch, ...
            cd_chainLengths_expanded(epoch), false, ...
...
fprintf('\r      Epoch %i Batch %i: TRAIN %.3g, VALID %.3g, elapsed %.3gm      ', ...
               epoch, batch, ...
            trainGoodnessAvg_exclWeightPenalty_currChunk, ...              
               validGoodnessAvg_exclWeightPenalty, ...
               trainElapsedTime_numMins);
